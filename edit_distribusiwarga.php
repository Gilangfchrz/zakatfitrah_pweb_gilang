...
<div class="content">
    ...
    <div>
        <p>Edit informasi Distribusi Warga :</p>
        <table border="1">
            <?php
            $id = $_GET['id'];
            $data = mysqli_query($con,"select * from mustahik_warga where id='$id'");
            while($d = mysqli_fetch_array($data)){
                ?>
                <form method="post" action="update_distribusiwarga.php">
                    <table>
                        <tr>
                            <td>ID</td>
                            <td><input type="text" name="id_mustahikwarga" value="<?php echo $d['id_mustahikwarga']; ?>"></td>
                        </tr>
                        <tr>
                            <td>Nama</td>
                            <td>
